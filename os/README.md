...
- IPC는 Inter-Process Communication의 약자로 프로세스간 통신을 의미한다. 프로세스는 커널이 제공하는 IPC 설비를 이용해 프로세스간 통신을 할 수 있다.

- IPC 설비의 종류는 여섯가지가 있다.
- 첫 번째는, PIPE(익명 파이프)이다. PIPE는 두 프로세스간 파이프를 연결해서 통신을 하는 방식이다. 여기서 한 프로세스는 쓰기만 가능하고 다른 프로세스는 읽기만 가능하다는 특징이 있다. 한쪽 방향으로만 통신이 가능하기 때문에 반이중 통신이라고 부르기도 한다. PIPE는 간단하게 사용할 수 있다는 장점이 있다.
- 두 번째는, Named PIPE(FIFO)이다. PIPE는 통신하는 프로세스가 명확할 경우 사용하는 반면, Named PIPE는 전혀 모르는 사이의 프로세스들의 통신에 사용한다. 익명 PIPE는 부모가 동일한 프로세스들 사이에서만 통신이 가능하지만 Named PIPE는 부모 프로세스에 상관없이 프로세스들 사이의 통신을 할 수 있다는 점이 특징이다. 이는 프로세스 통신을 위해 mkfifo함수를 이용해 파일을 생성하기 때문에 가능하다. 하지만, 익명 PIPE와 동일하게 동시에 읽기/쓰기가 불가능하다. 이는 두 개의 파일을 읽기 전용, 쓰기 전용으로 만들어서 해결할 수 있습니다. 전이중 통신을 위해서는 두 개의 fifo 파일을 만들어서 사용해야한다.
- 세 번째는, 메세지 큐이다. 메세지 큐는 선입선출의 형태로 통신이 이루어지는 점에서 Named PIPE와 동일하다. 차이점은 Named PIPE가 데이터의 흐름이라면 메세지 큐는 메모리 공간이라는 점이다. 이는 여러 개의 프로세스가 메세지 큐의 데이터에 접근할 수 있음을 의미한다.
- 네 번째는, 공유메모리이다. 앞서 PIPE, Named PIPE, 메세지 큐가 통신을 이용해 데이터를 주고받는다면 공유메모리는 프로세스간 메모리 영역을 공유해서 사용할 수 있도록 지원한다. 프로세스가 공유 메모리 할당을 커널에 요청하면 커널은 해당 프로세스에 메모리 공간을 할당해준다. 이후 어떤 프로세스건 해당 메모리영역에 접근할 수 있다. 공유 메모리는 곧바로 메모리에 접근할 수 있기 때문에 IPC 방식 중 속도가 제일 빠릅니다.
- 다섯 번째는, 메모리 맵이다. 메모리 맵은 공유 메모리와 메모리를 공유한다는 점은 동일하다. 하지만 현재 열려져 있는 파일을 공유하는 점에서 차이가 있다. 열린 파일이 메모리에 올라가있으면 다른 프로세스가 해당 파일을 사용할 때 또 다시 파일을 열지 않고 공유한 상태로 사용하는 것이 더 효율적이다.
- 여섯 번째는 소켓이다. 소켓은 소켓을 만들어 통신하는 방법이다. 소켓 통신은 데이터 교환을 위해 양쪽 PC에서 각각 임의의 포트를 정하고 해당 포트 간의 대화를 통해 데이터를 주고 받는 방식이다. 이 때 각각 PC의 프로세스는 임의의 PORT를 맡아 데이터를 송수신한다.
