...
## 브라우저 주소창에 http://www.test.com 입력 후 렌더링되는 과정

1. local DNS 서버에 www.test.com에 해당하는 ip주소가 있는지 물어본다. 있다면 바로 해당 ip로 받아옴
2. local DNS 서버에 없다면 루트 DNS 서버에 물어보고 있으면 ip로 받아옴
3. 루트 DNS 서버에 없다면 .com을 관리하는 DNS 서버에 물어봄, 있으면 ip로 받아온다
4. .com을 관리하는 DNS 서버에 없다면, test.com을 관리하는 DNS 서버에 물어본다. 있다면 ip로 받아온다.
5. 목적지의 ip를 알면 TCP 통신을 통해 소켓을 개방
6. HTTP 프로토콜로 요청
7. 라우팅 중 프록시 서버를 만난다면 웹 캐시에 저장된 정보를 response 받는다.
8. 프록시 서버를 만나지 못해 www.test.com을 서빙하는 서버까지 간다면 서버에서 요청에 맞는 데이터를 response로 전송
9. 브라우져의 loader가 해당 response를 다운로드할지 말지 결정
10. 브라우져의 웹 엔진이 다운로드한 .html 파일을 파싱하여 DOM 트리를 결정
11. .html 파싱 중 script 태그를 만나면 파싱을 중단하는 것이 원칙(지연 가능)
12. script 태그에 있는 자원을 다운로드하여 처리가 와나료되면 다시 파싱
13. CSS parser가 역시 .css파일을 파싱하여 스타일 규칙을 DOM 트리에 추가하여 렌더 트리를 만든다.
14. 이 렌더트리를 기반으로 브라우져의 크기에 따라 각 노드들의 크기를 결정
15. 페인트한다: 렌더링 엔진이 배치를 시작
...
프로토콜 스택은 통신 중 오류가 발생했을 때, 이 제어 정보를 사용하여 고쳐 보내거나, 각종 상황을 조절하는 등 다양한 역할을 수행.
...
1. 패킷은 인터넷의 입구에 있는 액세스 회선에 의해 POP까지 운반된다.
...
굳이 서버까지 안가도 되는 경우를 골라냄

1. 패킷은 인터넷 핵심부를 통과하여 웹 서버 측의 LAN에 도착
...
1. 서버의 수만큼 공인 IP 주소가 필요함, 부하 분산을 위해 서버의 대수를 늘리기 위해서는 그만큼의 공인 IP 가 필요하다.
2. 균등하게 분산되지 않으면 모바일 사이트 등에서 문제가 될 수 있는데, 스마트폰의 접속은 캐리어 게이트웨이라고 하는 프록시 서버를 경유한다. 프록시 서버에서는 이름변환 결과가 일정 시간 동안 캐싱되므로 같은 프록시 서버를 경유하는 접속은 항상 같은 서버로 접속된다. 또한 PC 용 웹 브라우저도 DNS 질의 결과를 캐싱하기 때문에 균등하게 부하분산되지 않는다. DNS 레코드의 TTL 값을 짧게 설정함으로써 어느 정도 해소가 되지만, TTL에 따라 캐시를 해제하는 것은 아니므로 반드시 주의가 필요하다.
3. 서버가 다운되도 확인 불가 DNS 서버는 웹 서버의 부하나 접속 수 등의 상황에 따라 질의 결과를 제어할 수 없다. 웹 서버의 부하가 높아서 응답이 느려지거나 접속 수가 꽉 차서 접속을 처리할 수 없는 상황인지를 전혀 감지할 수가 없기 때문에 어떤 원인으로 다운되더라도 이를 검출하지 못하고 유저들에게 제공한다. 이때문에 유저들은 간혹 다운된 서버로 연결이 되기도 한다. DNS 라운드 로빈은 어디까지나 부하분산을 위한 방법이지 다중화 방법은 아니므로 다른 S/W와 조합해서 관리할 필요가 있다.

__Weighted round robin(WRR)__

각각의 웹 서버에 가중치를 가미해서 분산 비율을 변경한다. 물론 가중치가 큰 서버일수록 빈번하게 선택되므로 처리 능력이 높은 서버는 가중치를 높게 설정하는 것이 좋다.



__Least connection__
...
- http의 connectionless란 특징으로 서버는 클라이언트 식별이 불가(stateless)
- 매 번 인증하려면 번거롭기 때문에 일정시간 정보를 저장할 기술의 필요성 대두
...
- ![쿠키](README.assets/쿠키.PNG)

## 쿠키의 특징

- 클라이언트(브라우저) 로컬에 저장되며 키와 값이 들어있음

- 쿠키는 유효기간이 있음 서버가 정한 기간에 따라 유효함

- 쿠키는 클라이언트의 상태 정보를 로컬에 저장했다가 참조 단, 쿠키는 도메인에 따라 제한됨

  ex) 유튜브가 준 쿠키는 유튜브에만 보내짐

- 300개까지 저장 가능, 하나의 도메인당 20개의 값만 가질 수 있음, 하나의 쿠키 값은 4KB까지 저장

- Response Header에 Set-Cookie 속성을 사용하면 클라이언트에 쿠키를 만들 수 있다.

- 쿠키는 자동으로 Request header에 담겨 서버에 전송된다.

- 쿠키는 인증 외에도 여러가지 정보를 저장이 가능함 ex) 설정한 언어의 페이지



## 세션이란

클라이언트에 session cookie로 임의의 session id를 부여하고(e.g. uuid)그 id를 redis 등 db에 key로서 저장하고 client 요청에 따라 해당 key에 매핑된 value를 바꿔주면서 서비스 이용자의 state를 추적하는 방식



## 세션 작동 원리

1. Client가 ID와 PW를 이용해 웹 서버에 전송
2. Server에서 DB를 조회하여 ID, PW 검증
3. 사용자가 맞다면 세션 DB에 유저 생성, 이 때 생기는 고유한 숫자(session id) Client 브라우저에 반환
4. 같은 웹사이트의 다른 페이지로 이동할 때마다 쿠키에 이 session id를 담아서 보내면, Server에서 session id를 session db에서 검색(반복) => client님 환영합니다와 같은 메세지를 띄울 수 있음



## 장점

- 로그인된 유저의 모든 정보를 db에 저장하기 때문에 추가 기능을 설계할 수 있다.

  ex) 인스타에서 원하지 않는 디바이스 로그아웃 기능, 넷플릭스 계정 공유 숫자 제한



## 단점

- 세션은 모든 요청에 따라 사용자를 세션 db에 저장하기 때문에 사용자가 늘수록 필요로 하는 DB리소스도 늘어난다.



## JWT 토큰이란

- 쿠키는 웹에서만 존재, 그렇기 때문에 토큰이란 것이 나옴
- Json 포맷을 이용하여 사용자에 대한 속성을 저장하는 **Claim 기반의 Web Token

​	ex) QR코드

**Claim: payload에 담기는 정보의 한 조각, 여기서는 사용자에 대한 정보를 의미함.
...
1. Client가 ID와 PW를 이용해 웹 서버에 전송
2. Server에서 DB를 조회하여 ID, PW 검증
3. 사용자가 맞다면 사인 알고리즘(HMAC)을 이용하여 JWT 토큰 생성 후 브라우저에 반환
...
- 누구나 열람할 수 있기 때문에 민감한 정보를 담아선 안된다.
